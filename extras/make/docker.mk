...
docker.build: clean
	@echo $(MESSAGE) "Building stageironment: ${stage} ${service}"
	@if [ -z "${stage}" ] && [ -z "${service}" ]; then \
		...
		docker volume create "${PROJECT}"-"${stage}"-db; \
		docker volume create "${PROJECT}"-"${stage}"-broker; \
...
docker.down: clean
	@echo $(MESSAGE) "Down Services Stageironment: ${stage}"
	@if [ -z  "${stage}" ]; then \
...
docker.exec: clean
	@echo $(MESSAGE) "Exec Services Stageironment: ${stage}"
	@if [ -z "${stage}" ]; then \
